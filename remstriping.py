...
class striping_noise():
    ...
    def measure_fullimage_striping(self, fitdata, mask):
    ...
    def measure_striping(self, image, origfilename, thresh=None, apply_flat=True, mask_sources=True, save_patterns=False):
        ...
        try:
            pedestal = self.fit_sky(pedestal_data)
        ...
        for amp in ['A','B','C','D']:
            ...
            hstriping_amp = self.collapse_image(ampdata, ampmask, dimension='y')
    ...
    #        outsci[np.where(wref)] = 0
            ...
            # add history entry
            time = datetime.now()
            stepdescription = 'Removed horizontal,vertical striping; remstriping.py %s'%time.strftime('%Y-%m-%d %H:%M:%S') + f"with threhold{self.MASKTHRESH}" + f", apply flat:{apply_flat}"
