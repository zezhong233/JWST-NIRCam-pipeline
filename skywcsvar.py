...
class sky_wcs_var_class():
    ...
    def fit_sky(self, data, plot_sky=False, ax=None, color='C1', **kwargs):
        ...
        popt,pcov = curve_fit(self.gaussian, bc, h, p0=p0)

        if plot_sky:
            ax.plot(bc, self.gaussian(bc, *popt), color, **kwargs)

        return popt[1]


    def run_background_subtraction(self,fitsfile):
        ...
        bs.do_background_subtraction(self.INPUTDIR, fitsfile, outputpath = self.OUTPUTDIR)


    def process(self,cal, plot_sky=False):
        ...
        self.run_background_subtraction(cal)
        ...
        try:
            sky = self.fit_sky(data, plot_sky=plot_sky, ax=ax, color='C1', 
                        alpha=0.5, lw=2)
        ...
        try:
            sky = self.fit_sky(clipped, plot_sky=plot_sky, ax=ax, color='C2', 
                        alpha=0.6, lw=1)
        except RuntimeError as e:
            print('!!! Error %s !!!'%cal)
            err = open('errors.list', 'a')
        ...
        fitsfile = cal.replace('%s.fits'%self.FILE_SUFFIX, '%s.fits'%self.OUT_SUFFIX)
        
        sv.read_file(self.INPUTDIR, fitsfile)
