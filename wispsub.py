...
class wispsub():
    ...
    def create_seg_map(self,data, segname='segm.fits', saveseg=False):
        ...
        kernel_sigma = mask_filter_fwhm * gaussian_fwhm_to_sigma
        kernel = Gaussian2DKernel(kernel_sigma, x_size=mask_filter_size,y_size=mask_filter_size)
        con_im = convolve(im, kernel)
        sigma_clip = SigmaClip(sigma=3.0, maxiters=5)
        threshold = detect_threshold(im, mask_nsigma, sigma_clip = sigma_clip)
        segm = detect_sources(con_im, threshold, mask_npixels)
        selem = np.ones((11, 11))
        mask = ndimage.binary_dilation(segm.data.astype(bool), selem) 
    ...
    def calc_variance(self,data, template, coeff):
    ...
    def fit_wisp_feature(self, image, origfilename, apply_flat=True, fit_scaling=True, scale=1.0):
        ...
        if fit_scaling:
            ...
            # source detection
            segfilename = os.path.join(self.OUTPUTDIR, image.replace('_rate.fits',
                                                                '_segm_cold.fits'))
            segm = self.create_seg_map(model, segfilename, saveseg=True)
            ...
            for i,c in enumerate(coeffs):
                variance_mad[i] = self.calc_variance(imseg, wispseg, c)
            ...
            outplot = os.path.join(self.OUTPUTDIR, 
    ...
    #        plt.show()
        ...
        shutil.copy2(os.path.join(self.INPUTDIR,image), 
                    os.path.join(self.OUTPUTDIR,origfilename))
        ...
        output = os.path.join(self.OUTPUTDIR, image)
        model.save(output)
    ...
    def main(self):
        ...
        self.fit_wisp_feature(args.image, origfilename, apply_flat=True, 
